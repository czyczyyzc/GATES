...
from engine.old.trainer_search import Trainer
...
def main(args):
    ...
    model = models.create(args.arch, num_classes=len(train_dataset.classes), init_channels=args.init_channels,
                          layers=args.layers, norm_layer=norm_layer)
    if args.distributed:
    ...
    # Create Trainer
    trainer = Trainer(model, netw_optimizer, arch_optimizer, args.unrolled, args.distributed)
